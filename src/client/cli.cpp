...
std::vector<std::string> Cli::getAuthorPaperIds()
{
    std::vector<std::string> paperIds;
    
    if (sessionId_.empty())
    {
        return paperIds; // 未登录，返回空列表
    }
    ...
    if (auto resp = sendRequest(payload))
    {
        ...
        if (resp->type == osp::protocol::MessageType::CommandResponse)
        {
            if (resp->payload.contains("ok") && resp->payload["ok"].get<bool>())
            {
                if (resp->payload.contains("data") && resp->payload["data"].contains("papers"))
                {
                    const auto& papers = resp->payload["data"]["papers"];
                    if (papers.is_array())
                    {
                        for (const auto& paper : papers)
                        {
                            ...
                            {
                                paperIds.push_back(std::to_string(paper["id"].get<std::uint32_t>()));
                            }
                        }
                    }
                }
...
void Cli::printAuthorNumericMenu() const
{
    ...
    std::cout << "  2) 提交修订版本 (SUBMIT_REVISION)\n";
    std::cout << "  3) 查看我的论文列表 (LIST_PAPERS)\n";
    std::cout << "  4) 查看论文详情 (GET_PAPER)\n";
    std::cout << "  5) 查看评审意见/状态 (LIST_REVIEWS)\n";
...
bool Cli::handleAuthorMenuInput(const std::string& line)
{
    ...
    if (authorWizard_ != AuthorWizard::None)
    {
        ...
        {
            std::string paperId = t;
            
            // 允许用户输入 'm' 返回菜单
            if (paperId == "m" || paperId == "M")
            {
            ...
            // 验证 paper ID 是否属于当前作者
            auto validPaperIds = getAuthorPaperIds();
            bool isValid = false;
            for (const auto& validId : validPaperIds)
            {
                if (validId == paperId)
                {
                    isValid = true;
            ...
            if (!isValid)
            {
                if (validPaperIds.empty())
                {
                    std::cout << "错误：您还没有提交过任何论文。请先提交新论文。\n";
                }
                else
                {
                    std::cout << "错误：论文 ID " << paperId << " 不属于您，或者该论文不存在。\n";
                    std::cout << "您的论文 ID 列表：";
                    for (size_t i = 0; i < validPaperIds.size(); ++i)
                    {
                        std::cout << validPaperIds[i];
                        if (i < validPaperIds.size() - 1)
                        {
                            std::cout << ", ";
                        }
                    }
                ...
                std::cout << "请重新输入有效的 paper_id，或输入 m 返回菜单: ";
                return true;
            }
            
            auto payload = buildJsonPayload("GET_PAPER " + paperId);
            if (auto resp = sendRequest(payload))
            {
                printResponse(*resp);
            }
        ...
        case AuthorWizard::ViewReviewsAskPaperId:
        {
            ...
            auto payload = buildJsonPayload("LIST_REVIEWS " + paperId);
            if (auto resp = sendRequest(payload))
            {
                printResponse(*resp);
            }
        ...
        case AuthorWizard::PostViewReviewsPrompt:
            ...
            authorWizard_ = AuthorWizard::None;
            return true;
        case AuthorWizard::SubmitRevisionAskPaperId:
        {
            std::string paperId = t;
            ...
            tempTitle_ = paperId; // 临时存储 paper_id
            std::cout << "输入修订内容（可包含空格）: ";
            authorWizard_ = AuthorWizard::SubmitRevisionAskContent;
            return true;
        }
        case AuthorWizard::SubmitRevisionAskContent:
        {
            const std::string content = t;
            if (content.empty())
            {
            ...
            auto payload = buildJsonPayload("SUBMIT_REVISION " + tempTitle_ + " " + content);
            if (auto resp = sendRequest(payload))
            {
                printResponse(*resp);
            }
            ...
            authorWizard_ = AuthorWizard::PostSubmitRevisionPrompt;
            return true;
        }
        case AuthorWizard::PostSubmitRevisionPrompt:
            if (t == "c" || t == "C")
            {
                authorWizard_ = AuthorWizard::SubmitRevisionAskPaperId;
                std::cout << "提交修订版本，输入 paper_id: ";
                return true;
            }
            if (t == "m" || t == "M")
            {
